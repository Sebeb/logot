...
A column whose prefix addresses a registered widget, or whose preview names one, embeds that widget's `"palette"` instance. Placement follows the column's phase. A preview column is a read-only look ahead, so its widget is pinned between the header and the scrolling region and stays on screen at every scroll position. The current and past columns are browsed, so their widget is inline: it occupies a leading slot of the scrolling region, is measured for row capacity while on screen, and is tracked by the row scrollbar. Scroll positions are therefore addressed in slots rather than row indices, and one step from the top scrolls only the widget away, leaving the first row where it was. That slot exists only when showing the widget costs the rows their space; when every row already fits alongside it, the column does not scroll and the widget cannot be scrolled away.
